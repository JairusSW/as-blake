...
#
# Flags:
#   --v8        Run under V8 / d8 (default if no runtime selected)
#   --wavm      Run under WAVM (requires `wavm` in PATH)
...
if [[ $RUN_V8 -eq 0 && $RUN_WAVM -eq 0 && $RUN_WAZERO -eq 0 ]]; then
  RUN_V8=1
fi
...
mkdir -p ./build/logs/as/{v8,wavm,wazero}
...
build_v8() {
  ...
  npx asc "$file" -o "${output}.tmp" -O3 --converge --noAssert --uncheckedBehavior always --runtime incremental --enable bulk-memory --exportStart start --exportRuntime "${MEMORY_ASC_ARGS[@]}"
  optimize_or_fallback "${output}.tmp" "$output" --enable-bulk-memory --enable-nontrapping-float-to-int --enable-tail-call -tnh -iit -ifwl -s 0 -O4
}
...
build_wasi() {
  ...
  npx asc "$file" -o "$output" -O3 --converge --noAssert --uncheckedBehavior always --runtime incremental --config ./node_modules/@assemblyscript/wasi-shim/asconfig.json --use "$runtime_flag=1" --enable bulk-memory --enable sign-extension --exportRuntime "${MEMORY_ASC_ARGS[@]}"
}
...
build_wasi_simd() {
  ...
  npx asc "$file" -o "$output" -O3 --converge --noAssert --uncheckedBehavior always --runtime incremental --config ./node_modules/@assemblyscript/wasi-shim/asconfig.json --use "$runtime_flag=1" --enable bulk-memory --enable sign-extension --enable simd --exportRuntime "${MEMORY_ASC_ARGS[@]}"
}
...
for file in "${FILES[@]}"; do
  ...
  # Files matching *simd* need --enable simd at compile time
  needs_simd=0
  [[ "$filename" == *simd* ]] && needs_simd=1

  if [[ $RUN_V8 -eq 1 ]]; then
    wasm_arg="${filename%.ts}.v8.wasm"
    if [[ $needs_simd -eq 1 ]]; then
      build_v8_simd "$file" "${output}.v8.wasm"
    else
  ...
  if [[ $RUN_WAVM -eq 1 ]]; then
    ...
    if [[ $needs_simd -eq 1 ]]; then
      build_wasi_simd "$file" "${output}.wavm.wasm" "AS_BENCH_RUNTIME_WAVM"
    else
  ...
  if [[ $RUN_WAZERO -eq 1 ]]; then
    ...
    if [[ $needs_simd -eq 1 ]]; then
      build_wasi_simd "$file" "${output}.wazero.wasm" "AS_BENCH_RUNTIME_WAZERO"
    else
