...
const BENCH_RUNTIME_STDOUT: bool = BENCH_RUNTIME_WAVM || BENCH_RUNTIME_WAZERO;
...
export function bench(
  ...
  let log =
    "   Completed benchmark in " +
    Math.round(elapsed).toString() +
    "ms at " +
    Math.round(opsPerSecond).toString() +
    " ops/s (" +
    fixed1(nsPerOp) +
    " ns/op)";
  ...
  if (BENCH_MEMORY) {
    ...
    log +=
      "\n   mem: base=" +
      memBaselineBytes.toString() +
      " peak=" +
      memPeakBytes.toString() +
      " retained=" +
      memRetainedBytes.toString() +
      " grew=+" +
      grew.toString() +
      " inflight=" +
      inflightDelta.toString() +
      " net=" +
      netDelta.toString() +
      " postGC=" +
      fixed1(postGcMs) +
...
export function dumpToFile(suite: string): void {
  const fileName =
    "./build/logs/as/" + benchRuntimeName() + "/" + suite + ".as.json";
...
function serializeResult(): string {
  ...
  return (
    ...
    r.nsPerOp.toString() +
    ',"opsPerSecond":' +
    r.opsPerSecond.toString() +
    ',"mbps":' +
    r.mbps.toString() +
    ',"gbps":' +
    r.gbps.toString() +
    ',"memoryBaselineBytes":' +
    r.memoryBaselineBytes.toString() +
    ',"memoryPeakBytes":' +
    r.memoryPeakBytes.toString() +
    ',"memoryRetainedBytes":' +
    r.memoryRetainedBytes.toString() +
    ',"memoryPostGcMs":' +
    r.memoryPostGcMs.toString() +
    ',"heapDetail":' +
    (r.heapDetail.length > 0 ? r.heapDetail : "{}") +
    "}"
  );
}
